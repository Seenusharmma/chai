import { MenuItem, CategoryInfo, Category } from '../types';

export const categories: CategoryInfo[] = [
  { id: 'biriyani', name: 'Biriyani', icon: 'Rice' },
  { id: 'chicken', name: 'Chicken', icon: 'Drumstick' },
  { id: 'mutton', name: 'Mutton', icon: 'Drumstick' },
  { id: 'egg', name: 'Egg', icon: 'Egg' },
  { id: 'veg', name: 'Veg', icon: 'Carrot' },
  { id: 'rice', name: 'Rice', icon: 'Rice' },
  { id: 'roti', name: 'Roti', icon: 'Circle' },
  { id: 'roll', name: 'Roll', icon: 'Wrap' },
  { id: 'soup', name: 'Soup', icon: 'Soup' },
  { id: 'noodles', name: 'Noodles', icon: 'Noodles' },
  { id: 'bread', name: 'Bread', icon: 'Bread' },
  { id: 'sandwich', name: 'Sandwich', icon: 'Sandwich' },
  { id: 'burger', name: 'Burger', icon: 'Burger' },
  { id: 'momo', name: 'Momo', icon: 'Dumpling' },
  { id: 'salad', name: 'Salad', icon: 'Salad' },
  { id: 'tea', name: 'Tea', icon: 'Cup' },
  { id: 'coffee', name: 'Coffee', icon: 'Coffee' },
  { id: 'mocktails', name: 'Mocktails', icon: 'Glass' },
  { id: 'maggie', name: 'Maggie', icon: 'Noodles' },
];

export const menuItems: MenuItem[] = [
  {
    id: '1',
    name: 'Espresso',
    description: 'Rich and bold espresso shot',
    price: 3.50,
    image: '/images/espresso.jpg',
    category: 'coffee',
    rating: 4.8,
    isPopular: true,
    isFeatured: true,
  },
  {
    id: '2',
    name: 'Cappuccino',
    description: 'Classic cappuccino with velvety foam',
    price: 4.50,
    image: '/images/cappuccino.jpeg',
    category: 'coffee',
    rating: 4.9,
    isPopular: true,
    isFeatured: true,
  },
  {
    id: '3',
    name: 'Latte',
    description: 'Smooth latte with steamed milk',
    price: 4.75,
    image: '/images/latte.jpg',
    category: 'coffee',
    rating: 4.7,
    isPopular: true,
  },
  {
    id: '4',
    name: 'Matcha Latte',
    description: 'Premium matcha green tea latte',
    price: 5.25,
    image: '/images/matcha.jpg',
    category: 'tea',
    rating: 4.6,
    isPopular: true,
  },
  {
    id: '5',
    name: 'Croissant',
    description: 'Buttery, flaky French croissant',
    price: 3.75,
    image: '/images/croissant.jpg',
    category: 'bread',
    rating: 4.8,
    isPopular: true,
  },
  {
    id: '6',
    name: 'Chocolate Muffin',
    description: 'Rich chocolate muffin with chocolate chips',
    price: 4.25,
    image: '/images/muffin.jpg',
    category: 'bread',
    rating: 4.7,
  },
  {
    id: '7',
    name: 'Avocado Toast',
    description: 'Fresh avocado on artisan bread',
    price: 7.50,
    image: '/images/avocado-toast.jpg',
    category: 'bread',
    rating: 4.9,
  },
  {
    id: '8',
    name: 'Breakfast Sandwich',
    description: 'Egg, cheese, and bacon on a croissant',
    price: 6.75,
    image: '/images/breakfast-sandwich.jpg',
    category: 'sandwich',
    rating: 4.8,
  },
  {
    id: '9',
    name: 'Green Eggplant',
    description: 'Healthy green smoothie bowl',
    price: 6.50,
    image: '/images/green-bowl.jpg',
    category: 'salad',
    rating: 4.5,
  },
  {
    id: '10',
    name: 'Iced Coffee',
    description: 'Refreshing cold brew coffee',
    price: 4.00,
    image: '/images/iced-coffee.jpg',
    category: 'coffee',
    rating: 4.6,
  },
  {
    id: '11',
    name: 'Caramel Macchiato',
    description: 'Sweet caramel espresso drink',
    price: 5.50,
    image: '/images/caramel-macchiato.jpg',
    category: 'coffee',
    rating: 4.8,
  },
  {
    id: '12',
    name: 'Cheese Cake',
    description: 'Creamy New York style cheesecake',
    price: 5.75,
    image: '/images/cheesecake.jpg',
    category: 'bread',
    rating: 4.9,
  },
  {
    id: 'special-1',
    name: 'Chicken Biriyani',
    description: 'Fragrant basmati rice cooked with spiced chicken and aromatic herbs',
    price: 250.00,
    image: 'https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?w=500&h=500&fit=crop',
    category: 'biriyani',
    rating: 4.9,
    isPopular: true,
    isFeatured: true,
  },
  {
    id: 'special-2',
    name: 'Mutton Khasa',
    description: 'Tender mutton pieces cooked in rich, aromatic gravy with spices',
    price: 320.00,
    image: 'https://static.vecteezy.com/system/resources/thumbnails/063/143/487/small/indian-food-recipes-indian-food-recipes-photo.jpg',
    category: 'mutton',
    rating: 4.8,
    isPopular: true,
    isFeatured: true,
  },
  {
    id: 'special-3',
    name: 'Hyderabadi Biriyani',
    description: 'Authentic Hyderabadi dum biriyani with tender meat and saffron',
    price: 280.00,
    image: 'https://images.unsplash.com/photo-1589302168068-964664d93dc0?w=500&h=500&fit=crop',
    category: 'biriyani',
    rating: 4.9,
    isFeatured: true,
  },
  {
    id: 'special-4',
    name: 'Chicken 65',
    description: 'Spicy, crispy fried chicken appetizer with aromatic spices',
    price: 180.00,
    image: 'https://images.unsplash.com/photo-1601050690597-df0568f70950?w=500&h=500&fit=crop',
    category: 'chicken',
    rating: 4.7,
    isPopular: true,
  },
  {
    id: 'special-5',
    name: 'Mutton Biriyani',
    description: 'Premium mutton biriyani with slow-cooked flavorful meat',
    price: 350.00,
    image: 'https://images.unsplash.com/photo-1563379091339-03b21ab4a4f8?w=500&h=500&fit=crop',
    category: 'biriyani',
    rating: 4.8,
    isFeatured: true,
  },
  {
    id: 'special-6',
    name: 'Chicken Tikka Masala',
    description: 'Grilled chicken in creamy, spiced tomato-based curry',
    price: 220.00,
    image: 'https://images.unsplash.com/photo-1565557623262-b51c2513a641?w=500&h=500&fit=crop',
    category: 'chicken',
    rating: 4.6,
  },
];
